                ...
                <tbody class="text-gray-700 text-sm">
                    @forelse($alats as $index => $alat)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="py-3 px-4 border-b text-center">{{ $alats->firstItem() + $index }}</td>
                            <td class="py-3 px-4 border-b">
                                @if($alat->gambar)
                                    <img src="{{ asset($alat->gambar) }}" alt="{{ $alat->nama_alat }}" class="w-12 h-12 object-cover rounded-md border">
                                @else
                            ...
                            <td class="py-3 px-4 border-b">
                                <span class="px-2.5 py-1 text-xs rounded-full font-semibold
                                    @if($alat->status_kondisi == 'Baik') bg-emerald-100 text-emerald-800
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-4 text-center text-gray-500">Belum ada data alat.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
